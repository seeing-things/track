...
class TestModelParamSet(unittest.TestCase):
    ...
    def test_apply_guide_cam_alignment_error(self):
        ...
        for _ in range(20):
            original_param_set = ModelParamSet(
                ModelParameters(
            ...
            transformed_param_set = apply_guide_cam_alignment_error(
                old_params=original_param_set, guide_cam_align_error=align_error
            )

            restored_param_set = apply_guide_cam_alignment_error(
                old_params=transformed_param_set, guide_cam_align_error=Angle(0 * u.deg)
            )
    ...
    def test_camera_tilt(self):
        ...
        for _ in range(20):
            ...
            coord = UnitSphericalRepresentation(
                np.random.uniform(0.0, 360.0) * u.deg, np.random.uniform(-90.0, 90.0) * u.deg
            )
