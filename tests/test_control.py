...
class TestSmallestAllowedError(unittest.TestCase):
    def test_positive_error(self):
        error = smallest_allowed_error(mount_enc_position=1.0, target_enc_position=2.0)
        assert_allclose(error, 1.0)

    def test_negative_error(self):
        error = smallest_allowed_error(mount_enc_position=2.0, target_enc_position=1.0)
        assert_allclose(error, -1.0)

    def test_wrapping(self):
        error = smallest_allowed_error(mount_enc_position=359.0, target_enc_position=0.0)
        assert_allclose(error, 1.0)
